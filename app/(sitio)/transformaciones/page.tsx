...
export default async function TransformacionesPage() {
  const [intro, casos] = await Promise.all([
    getContenido<SeccionEncabezado>("transformaciones_intro"),
    getCasosGaleria(),
  ]);
    ...
    <div className="pb-24">
      <section className="container pt-28 text-center md:pt-32">
        <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground">
          {intro?.kicker ?? "Transformaciones"}
        </p>
        <h1 className="mx-auto mt-4 max-w-2xl text-fluid-3xl">
          {intro?.titulo ?? "Resultados reales en el tiempo"}
        </h1>
        ...
        {casos.length === 0 ? (
          <div className="mx-auto mt-12 max-w-md rounded-lg border border-dashed border-border p-10 text-center">
            <p className="text-sm text-muted-foreground">
              Las transformaciones se publican desde el portal, con su
              consentimiento, y aparecerán aquí en cuanto estén listas.
            </p>
          </div>
        ) : (
